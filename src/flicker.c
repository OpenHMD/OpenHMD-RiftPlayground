...
static int pattern_find_id(const uint16_t *patterns, int num_patterns,
...
static int pattern_get_phase(const uint16_t *patterns, int num_patterns,
			     uint16_t pattern)
{
	int i, j;
...
void flicker_process(struct blob *blobs, int num_blobs,
		     int skipped, int num_patterns, const uint16_t *patterns)
{
	struct blob *b;
	int success = 0;
	int phase = flicker_phase;
	...
	for (b = blobs; b < blobs + num_blobs; b++) {
		...
		success += pattern_find_id(patterns, num_patterns, pattern,
					   &b->led_id);
	}
	...
	if (success < 0 || phase < 0) {
		...
		for (b = blobs; b < blobs + num_blobs; b++) {
			int phase = pattern_get_phase(patterns, num_patterns,
						      b->pattern);
			if (phase >= 0)
				phase_error[phase]++;
		}
